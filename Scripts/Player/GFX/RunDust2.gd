...
func _process(delta):
	if (round(get_parent().get_parent().Speed) == 0 or not get_parent().get_parent().Midair == 0) or (not abs((get_parent().get_parent().Speed)) >= 600 and not get_parent().get_parent().braking == 0):
		one_shot = true
